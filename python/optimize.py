...
def fmin(func, x0, args=(), xtol=1e-4, ftol=1e-4, maxiter=None, maxfun=None, fulloutput=0, printmessg=1):
    ...
    ind = np.argsort(fsim)
    fsim = np.take(fsim,ind)     # sort so sim[0,:] has the lowest function value
...
def line_search(f, fprime, xk, pk, gfk, args=(), c1=1e-4, c2=0.9, amax=50):
    ...
    derphi_a0 = np.dot(apply(fprime,(xk+alpha0*pk,)+args),pk)
    gc = gc + 1
    ...
    while 1:
        ...
        derphi_a1 = np.dot(apply(fprime,(xk+alpha1*pk,)+args),pk)
        gc = gc + 1
...
def approx_fprime(xk,f,*args):
    f0 = apply(f,(xk,)+args)
    grad = np.zeros((len(xk),),'d')
    ei = np.zeros((len(xk),),'d')
...
def fminBFGS(f, x0, fprime=None, args=(), avegtol=1e-5, maxiter=None, fulloutput=0, printmessg=1):
    ...
    xk = x0
    sk = [2*gtol]
    while (np.add.reduce(abs(gfk)) > gtol) and (k < maxiter):
        pk = -np.dot(Hk,gfk)
...
def fminNCG(f, x0, fprime, fhess_p=None, fhess=None, args=(), avextol=1e-5, maxiter=None, fulloutput=0, printmessg=1):
    ...
    xk = x0
    k = 0
    while (np.add.reduce(abs(update)) > xtol) and (k < maxiter):
        # Compute a search direction pk by applying the CG method to
        #  del2 f(xk) p = - grad f(xk) starting from 0.
        b = -apply(fprime,(xk,)+args)
        gcalls = gcalls + 1
        maggrad = np.add.reduce(abs(b))
        ...
        while np.add.reduce(abs(ri)) > termcond:
            if fhess is None:
                if approx_hessp:
